...
def im(reqs):
	for req in reqs:
		...
		cursor = cnx.cursor()
		query = "SELECT * FROM request WHERE publicBodyId = 1 AND requestId = %s"
		cursor.execute(query, (req['iden'],))
		...
		cnx = mysql.connector.connect(user='foiawiki', password='579dVZq8BBa3XRcc', database='foiawiki')
		cursor = cnx.cursor()
		query = "INSERT INTO request VALUES (1, %s, %s, %s, %s, %s, NULL)"
		tagQuery = "INSERT INTO requestTag VALUES (%s, %s)"
		print req
		for tag in req['tags']:
			tagArgs = (req['iden'], tag)
			print tagArgs
			cursor.execute(tagQuery, tagArgs)
		cursor.execute(query, (req['iden'], req['type'], req['title'], req['date'], req['doc']))
		cnx.commit()
		cursor.close()
		cnx.close()
